...
def get_mean_image_from_collection(collection):
    def get_mean_image(timestamp):
        ...
        pri = mean_image.normalizedDifference(['B4', 'B5']).rename('PRI')
        return pri.set('system:time_start', date.millis())
...
def get_site(site_file):
    out_file_normal = f"{site_file}_pri_mean_timelapse.gif"
    out_file_date = f"{site_file}_pri_mean_timelapse_date.gif"
