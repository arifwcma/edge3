...
def calculate_difference_image(collection, timestamp):
    ...
    filtered1 = collection.filterDate(date1, date2).filter(ee.Filter.listContains('system:band_names', 'B5')).filter(ee.Filter.listContains('system:band_names', 'B8A'))
    filtered2 = collection.filterDate(date2, date3).filter(ee.Filter.listContains('system:band_names', 'B5')).filter(ee.Filter.listContains('system:band_names', 'B8A'))
    mean_image1 = filtered1.mean().expression(
        '(B8A / B5) - 1', {
            'B5': filtered1.mean().select('B5'),
            'B8A': filtered1.mean().select('B8A')
        }).rename('Chlorophyll_Index')
    mean_image2 = filtered2.mean().expression(
        '(B8A / B5) - 1', {
            'B5': filtered2.mean().select('B5'),
            'B8A': filtered2.mean().select('B8A')
        }).rename('Chlorophyll_Index')
    diff_image = mean_image2.subtract(mean_image1).rename('Chlorophyll_Index_DIFF')
...
def generate_gif_for_site(site_file):
    ...
    composites_chlorophyll_index_diff = ee.ImageCollection(frames)
    video_args = {
        'dimensions': 720,
        'region': roi.geometry(),
        'framesPerSecond': 0.5,
    ...
    gif_path = os.path.join("temp", f"{name}_Chlorophyll_Index_raw.gif")
    geemap.download_ee_video(composites_chlorophyll_index_diff, video_args, gif_path)
    ...
    output_path_with_text = os.path.join("diff", f"{name}_Chlorophyll_Index.gif")
