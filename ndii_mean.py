...
def get_mean_image_from_collection(collection):
    def get_mean_image(timestamp):
        ...
        ndii = mean_image.normalizedDifference(['B8', 'B11']).rename('NDII')
        return ndii.set('system:time_start', date.millis())
...
def get_site(site_file):
    out_file_normal = f"{site_file}_ndii_mean_timelapse.gif"
    out_file_date = f"{site_file}_ndii_mean_timelapse_date.gif"
    if file_exists(site_file, "ndii"):
    ...
    composites_ndii = ee.ImageCollection(frames)
    ...
    geemap.download_ee_video(composites_ndii, video_args, gif_path)
