...
def get_mean_image_from_collection(collection):
    def get_mean_image(timestamp):
        ...
        evi = mean_image.expression(
            '2.5 * ((B8 - B4) / (B8 + 6 * B4 - 7.5 * B2 + 1))',
            {
                'B8': mean_image.select('B8'),
                'B4': mean_image.select('B4'),
                'B2': mean_image.select('B2')
            }
        ).rename('EVI')
        return evi.set('system:time_start', date.millis())
...
def get_site(site_file):
    out_file_normal = f"{site_file}_evi_mean_timelapse.gif"
    out_file_date = f"{site_file}_evi_mean_timelapse_date.gif"
    if file_exists(site_file, "evi"):
    ...
    composites_evi = ee.ImageCollection(frames)
    ...
    geemap.download_ee_video(composites_evi, video_args, gif_path)
