...
def calculate_difference_image(collection, timestamp):
    ...
    filtered1 = collection.filterDate(date1, date2).filter(ee.Filter.listContains('system:band_names', 'B8A')).filter(ee.Filter.listContains('system:band_names', 'B8'))
    filtered2 = collection.filterDate(date2, date3).filter(ee.Filter.listContains('system:band_names', 'B8A')).filter(ee.Filter.listContains('system:band_names', 'B8'))
    mean_image1 = filtered1.mean().normalizedDifference(['B8A', 'B8']).rename('NDII')
    mean_image2 = filtered2.mean().normalizedDifference(['B8A', 'B8']).rename('NDII')
    diff_image = mean_image2.subtract(mean_image1).rename('NDII_DIFF')
...
def generate_gif_for_site(site_file):
    ...
    composites_ndii_diff = ee.ImageCollection(frames)
    video_args = {
        'dimensions': 720,
        'region': roi.geometry(),
        'framesPerSecond': 0.5,
    ...
    gif_path = os.path.join("temp", f"{name}_NDII_raw.gif")
    geemap.download_ee_video(composites_ndii_diff, video_args, gif_path)
    ...
    output_path_with_text = os.path.join("diff", f"{name}_NDII.gif")
