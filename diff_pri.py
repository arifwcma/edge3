...
def calculate_difference_image(collection, timestamp):
    ...
    filtered1 = collection.filterDate(date1, date2).filter(ee.Filter.listContains('system:band_names', 'B3')).filter(ee.Filter.listContains('system:band_names', 'B4'))
    filtered2 = collection.filterDate(date2, date3).filter(ee.Filter.listContains('system:band_names', 'B3')).filter(ee.Filter.listContains('system:band_names', 'B4'))
    mean_image1 = filtered1.mean().normalizedDifference(['B3', 'B4']).rename('PRI')
    mean_image2 = filtered2.mean().normalizedDifference(['B3', 'B4']).rename('PRI')
    diff_image = mean_image2.subtract(mean_image1).rename('PRI_DIFF')
...
def generate_gif_for_site(site_file):
    ...
    composites_pri_diff = ee.ImageCollection(frames)
    video_args = {
        'dimensions': 720,
        'region': roi.geometry(),
        'framesPerSecond': 0.5,
    ...
    gif_path = os.path.join("temp", f"{name}_PRI_raw.gif")
    geemap.download_ee_video(composites_pri_diff, video_args, gif_path)
    ...
    output_path_with_text = os.path.join("diff", f"{name}_PRI.gif")
