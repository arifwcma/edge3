...
def get_mean_image_from_collection(collection):
    def get_mean_image(timestamp):
        ...
        ndwi = mean_image.normalizedDifference(['B3', 'B8']).rename('NDWI')
        return ndwi.set('system:time_start', date.millis())
...
def get_site(site_file):
    out_file_normal = f"{site_file}_ndwi_mean_timelapse.gif"
    out_file_date = f"{site_file}_ndwi_mean_timelapse_date.gif"
    if file_exists(site_file, "ndwi"):
    ...
    composites_ndwi = ee.ImageCollection(frames)
    ...
    geemap.download_ee_video(composites_ndwi, video_args, gif_path)
