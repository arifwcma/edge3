...
for site in sites:
    ...
    while start_date.millis().getInfo() < end_date.millis().getInfo():
        ...
        if ndwi_stats:
            avg_si = ndwi_stats.get('nd_mean', None)
            std = ndwi_stats.get('nd_stdDev', None)
            num = ndwi_stats.get('nd_count', None)
            data.append({
    ...
    csv_path = os.path.join(output_folder, f"{name}_ndwi_stats2.csv")
