...
def get_mean_image_from_collection(collection):
    def get_mean_image(timestamp):
        ...
        return ci.set('system:time_start', date.millis())
...
def get_site(site_file):
    out_file_normal = f"{site_file}_ci_mean_timelapse.gif"
    out_file_date = f"{site_file}_ci_mean_timelapse_date.gif"
    if file_exists(site_file, "ci"):
    ...
    composites_ci = ee.ImageCollection(frames)
    ...
    geemap.download_ee_video(composites_ci, video_args, gif_path)
