...
for site in sites:
    ...
    while start_date.millis().getInfo() < end_date.millis().getInfo():
        ...
        evi_stats = images.map(lambda img: img.expression(
        ...
        if evi_stats:
            avg_si = evi_stats.get('constant_stdDev', None)
            std = evi_stats.get('constant_mean', None)
            num = evi_stats.get('constant_count', None)
            data.append({
    ...
    csv_path = os.path.join(output_folder, f"{name}_evi_stats2.csv")
