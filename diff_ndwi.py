...
def calculate_difference_image(collection, timestamp):
    ...
    filtered1 = collection.filterDate(date1, date2).filter(ee.Filter.listContains('system:band_names', 'B8A')).filter(ee.Filter.listContains('system:band_names', 'B11'))
    filtered2 = collection.filterDate(date2, date3).filter(ee.Filter.listContains('system:band_names', 'B8A')).filter(ee.Filter.listContains('system:band_names', 'B11'))
    mean_image1 = filtered1.mean().normalizedDifference(['B8A', 'B11']).rename('NDWI')
    mean_image2 = filtered2.mean().normalizedDifference(['B8A', 'B11']).rename('NDWI')
    diff_image = mean_image2.subtract(mean_image1).rename('NDWI_DIFF')
...
def generate_gif_for_site(site_file):
    ...
    composites_ndwi_diff = ee.ImageCollection(frames)
    video_args = {
        'dimensions': 720,
        'region': roi.geometry(),
        'framesPerSecond': 0.5,
    ...
    gif_path = os.path.join("temp", f"{name}_NDWI_raw.gif")
    geemap.download_ee_video(composites_ndwi_diff, video_args, gif_path)
    dates_list = dates.map(lambda d: ee.Date(d).advance(90, "day").format('YYYY-MM-dd')).getInfo()
    frames_size = composites_ndwi_diff.size().getInfo()
    dates_list = dates_list[0:frames_size]
    dates_list = [f"{i+1}: {d}" for i, d in enumerate(dates_list)]
    output_path_with_text = os.path.join("diff", f"{name}_NDWI.gif")
