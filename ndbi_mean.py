...
def get_mean_image_from_collection(collection):
    def get_mean_image(timestamp):
        ...
        ndbi = mean_image.normalizedDifference(['B11', 'B8']).rename('NDBI')
        return ndbi.set('system:time_start', date.millis())
...
def get_site(site_file):
    out_file_normal = f"{site_file}_ndbi_mean_timelapse.gif"
    out_file_date = f"{site_file}_ndbi_mean_timelapse_date.gif"
    if file_exists(site_file, "ndbi"):
    ...
    composites_ndbi = ee.ImageCollection(frames)
    ...
    geemap.download_ee_video(composites_ndbi, video_args, gif_path)
