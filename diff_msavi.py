...
def calculate_difference_image(collection, timestamp):
    ...
    diff_image = mean_image2.subtract(mean_image1).rename('MSAVI_DIFF')
...
def generate_gif_for_site(site_file):
    ...
    composites_msavi_diff = ee.ImageCollection(frames)
    video_args = {
        'dimensions': 720,
        'region': roi.geometry(),
        'framesPerSecond': 0.5,
    ...
    gif_path = os.path.join("temp", f"{name}_MSAVI_raw.gif")
    geemap.download_ee_video(composites_msavi_diff, video_args, gif_path)
    ...
    output_path_with_text = os.path.join("diff", f"{name}_MSAVI.gif")
