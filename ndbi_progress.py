...
for site in sites:
    ...
    while start_date.millis().getInfo() < end_date.millis().getInfo():
        ...
        ndbi_stats = images.map(lambda img: img.normalizedDifference(['B11', 'B8'])).mean().reduceRegion(
        ...
        if ndbi_stats:
            avg_si = ndbi_stats.get('nd_mean', None)
            std = ndbi_stats.get('nd_stdDev', None)
            num = ndbi_stats.get('nd_count', None)
            data.append({
    ...
    csv_path = os.path.join(output_folder, f"{name}_ndbi_stats2.csv")
