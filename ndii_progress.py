...
for site in sites:
    ...
    while start_date.millis().getInfo() < end_date.millis().getInfo():
        ...
        ndii_stats = images.map(lambda img: img.normalizedDifference(['B8', 'B11'])).mean().reduceRegion(
        ...
        if ndii_stats:
            avg_si = ndii_stats.get('nd_mean', None)
            std = ndii_stats.get('nd_stdDev', None)
            num = ndii_stats.get('nd_count', None)
            data.append({
    ...
    csv_path = os.path.join(output_folder, f"{name}_ndii_stats2.csv")
